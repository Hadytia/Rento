...
class PenaltyController extends Controller
{
    ...
    public function index()
    {
        $penalties = DB::table('penalties as p')
            ...
            ->where('p.status', 1)
            ->select(
                'p.id', 'p.transaction_id', 'p.penalty_type', 'p.penalty_amount',
                'p.overdue_days', 'p.description', 'p.resolved', 'p.created_date',
                'p.created_by', 'p.last_updated_by', 'p.last_updated_date',
                't.trx_code', 't.rental_start', 't.rental_end', 't.trx_status',
                'u.name as customer_name', 'u.email as customer_email', 'pr.product_name'
            )
        ...
        $overdueTransactions = DB::table('transactions as t')
            ...
            ->groupBy('t.id', 't.trx_code', 't.rental_end', 't.total_days', 'u.name', 'u.email', 'pr.product_name')
            ->get();
    ...
    public function markResolved($id)
    {
        ...
        DB::table('penalties')->where('id', $id)->update(['resolved' => 1, 'status' => 1]);
    ...
    public function markFinished($id)
    {
        ...
        DB::table('penalties')->where('id', $id)->update(['is_deleted' => 1]);
        if ($penalty) {
            DB::table('transactions')->where('id', $penalty->transaction_id)->update(['trx_status' => 'Completed']);
        }
    ...
    public function apiIndex(Request $request)
    {
        try {
            $query = DB::table('penalties as p')
                ...
                ->where('p.status', 1)
                ->select(
                    'p.id', 'p.penalty_type', 'p.penalty_amount', 'p.overdue_days',
                    'p.description', 'p.resolved', 'p.created_date',
                    't.trx_code', 't.rental_start', 't.rental_end', 't.trx_status',
                    'u.name as customer_name', 'u.email as customer_email', 'pr.product_name'
                )
                ->orderBy('p.created_date', 'desc');
            ...
            return response()->json([
                'success' => false,
                'message' => 'Gagal mengambil data penalty.',
                'error'   => $e->getMessage(),
            ], 500);
        }
    }
    ...
    public function apiStore(Request $request)
    {
        try {
            ...
            return response()->json([
                'success' => false,
                'message' => 'Gagal menambahkan penalty.',
                'error'   => $e->getMessage(),
            ], 500);
        }
    }
